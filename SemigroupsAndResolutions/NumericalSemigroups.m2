...
///
restart
--commented out syzDecomp -- causing an error I don't understand
--seems it was used to study resolutions in the case of arf.
...
facetRays(List) := L -> (
    ...
    A:=apery L1;
    m:=A#"multiplicity";
    halfSpaces := coneEquations m; -- homogeneous cone eqns
    a:=matrix{aperySet A};
...
facetRaysMu=method()
facetRaysMu(List) := muL -> (
    --matrix whose columns are the rays of the homogeneous
...
semigroupsFromMatrix Matrix :=  M -> (
    ...
    --for example M might be the output of facetRays L
    r := numrows M;
    mult := r+1;
    c := numcols M;
    apply(c, j -> (
...
L = {6,9,13,16}
mm = mu L
assert(L == semigroupFromMu mm)
semigroupsFromMatrix facetRaysMu L
apply(semigroupsFromMatrix facetRaysMu L, ell -> aperySet ell)
assert(
    rank facetRays L == rank(transpose matrix {mu L}|facetRays L)
    )

semigroupsFromMatrix coneRays 4
facetRays {4,5}
mu {4,5}
facetRays (L = semigroupFromMu {2,2,4})
L
///
...
Description
  ...
  Text
   ...
   second matrix, M, when prepended by the multiplicity 3,
